...
const execAdduserinDB = function (index, callback) {
  ...
  // Authenticate on Admin-DB
    ...
    const addvar = { user: options.newdbuser, pwd: options.newdbpassword, roles: [{ role: 'readWrite', db: options.newdbname }]};

    adminDb.authenticate(config.admindbuser, config.admindbpassword);
    db.addUser(addvar);
    db.close();
