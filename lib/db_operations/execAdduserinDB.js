...
const execAdduserinDB = function (index, callback) {
  ...
  // Authenticate on Admin-DB
    ...
    db.open(function (err) {
      if (err) {
        return log.error('Error on open Database: ' + err);
      }

      const adminDb = db.admin();

      adminDb.authenticate(config.admindbuser, config.admindbpassword, function (err1) {
        if (err1) {
          log.error('Error on authenticate Admin-DB ' + err1);
          throw err1;
        }

        db.addUser(options.newdbuser, options.newdbpassword, function (err2) {
          if (err2) {
            log.error('Error on add User' + err2);
            throw err2;
          }
        });
      });
